...
class AsyncJsonlSessionStore(AsyncSessionStore):
    ...
    def _default_quanora_home(self) -> str:
        # Quanora primary env var, with legacy CHAINPEER_HOME fallback for backward-compat
        custom_home = os.getenv("QUANORA_HOME") or os.getenv("CHAINPEER_HOME")
        if custom_home:
            return os.path.abspath(os.path.expanduser(custom_home))
        # Prefer ~/.quanora; if it doesn't exist but legacy ~/.chainpeer does, keep using legacy path
        home = os.path.expanduser("~")
        new_home = os.path.join(home, ".quanora")
        legacy_home = os.path.join(home, ".chainpeer")
        if not os.path.isdir(new_home) and os.path.isdir(legacy_home):
            return os.path.abspath(legacy_home)
        return os.path.abspath(new_home)

    # Backward-compat alias (older code/tests may call _default_chainpeer_home)
